...
RSpec.describe 'entities/index', type: :view do
  ...
  let(:group) do
    Group.create!(name: 'Group 1', icon: 'fa-icon-1', user:).tap do |grp|
      grp.entities.create!(name: 'Entity 1', amount: 50, user:)
      grp.entities.create!(name: 'Entity 2', amount: 50, user:)
    end
  end
